...
def data_transform(path, episode_num, save_path):
    ...
    for i in range(episode_num):
        ...
        for j in range(0, left_gripper_all.shape[0]):
            ...
            # print("left_arm type/shape:", type(left_arm), getattr(left_arm, "shape", None))
            # print("right_arm type/shape:", type(right_arm), getattr(right_arm, "shape", None))
            # print("left_gripper:", type(left_gripper), left_gripper)
            # print("right_gripper:", type(right_gripper), right_gripper)

            # tmp = left_arm.tolist() + [left_gripper] + right_arm.tolist() + [right_gripper]
            # print("tmp len:", len(tmp))
            # print("tmp element types:", [type(x) for x in tmp])

            # state = np.array(left_arm.tolist() + [left_gripper.item()] + right_arm.tolist() + [right_gripper.item()])  # joints angle
...
if __name__ == "__main__":
    ...
    robotwin_root = policy_root.parents[1]
    dataset_root = Path(
        os.environ.get("ROBOTWIN_DATASET_ROOT", robotwin_root / "data_NAS" / "data" / "dataset")
    ).expanduser()
    load_dir = dataset_root / str(task_name) / str(setting)
    if not load_dir.is_dir():
        raise FileNotFoundError(
            f"Raw dataset directory not found: {load_dir}. "
            "Set ROBOTWIN_DATASET_ROOT to the local RoboTwin dataset root."
        )
    ...
    processed_data_root = Path(
        os.environ.get("OPENPI_PROCESSED_DATA_ROOT", policy_root / "processed_data")
    ).expanduser()
    target_dir = processed_data_root / f"{task_name}-{setting}-{expert_data_num}"
