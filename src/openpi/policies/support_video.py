...
class SupportVideoBank:
    ...
    def discover(
        ...
        *,
        ...
        if view is not None and Path(view).name != view:
            raise ValueError(f"view must be a directory name, got {view!r}")
        pattern = f"*/{view}/frames.npy" if view else "*/*/frames.npy"
        candidates = tuple(
            (frames_path.parent.parent.name, frames_path.parent.name)
            for frames_path in sorted(task_root.glob(pattern))
        )
        if not candidates:
            view_suffix = f" for view={view!r}" if view else ""
            raise FileNotFoundError(f"No support frames.npy found under: {task_root}{view_suffix}")
        return candidates
