...
class PaligemmaTokenizer:
    def __init__(self, max_len: int = 48):
        ...
        self._hand_side_token_ids = np.asarray(
            [
                token_id
                for text in ("left", " left", "Left", " Left", "right", " right", "Right", " Right")
                for token_id in self._tokenizer.encode(text)
            ],
            dtype=np.int32,
        )
        self._hand_token_ids = np.asarray(
            [token_id for text in ("hand", " hand", "Hand", " Hand") for token_id in self._tokenizer.encode(text)],
            dtype=np.int32,
        )
    ...
    def tokenize_caption_teacher_forcing(
        self, caption: str
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Tokenize a caption into shifted inputs and next-token targets.

        The returned arrays are padded to ``max_len``. Unlike prompt tokenization,
        captions are never silently truncated because that would remove supervised
        targets without making the data error visible.
        """
        cleaned_text = caption.strip().replace("_", " ").replace("\n", " ")
        if not cleaned_text:
            raise ValueError("Caption must not be empty.")

        sequence = self._tokenizer.encode(cleaned_text, add_bos=True, add_eos=True)
        input_tokens = sequence[:-1]
        target_tokens = sequence[1:]
        token_count = len(input_tokens)
        if token_count > self._max_len:
            raise ValueError(
                f"Caption teacher-forcing length ({token_count}) exceeds max length ({self._max_len}). "
                "Increase caption_max_len instead of truncating supervision."
            )

        pad_id = max(self._tokenizer.pad_id(), 0)
        padding = self._max_len - token_count
        input_tokens = np.asarray(input_tokens + [pad_id] * padding, dtype=np.int32)
        target_tokens = np.asarray(target_tokens + [pad_id] * padding, dtype=np.int32)
        input_mask = np.arange(self._max_len) < token_count
        loss_mask = input_mask.copy()

        side_mask = np.isin(target_tokens, self._hand_side_token_ids)
        followed_by_hand = np.concatenate([np.isin(target_tokens[1:], self._hand_token_ids), np.asarray([False])])
        hand_side_mask = np.logical_and(loss_mask, np.logical_and(side_mask, followed_by_hand))

        return input_tokens, input_mask, target_tokens, loss_mask, hand_side_mask
