...
if [[ "$#" -lt 4 ]]; then
    echo "Usage: bash run_eval_dataset_loss_trainlike.sh <train_config_name> <exp_name> <ckpt_step> <gpu_id> [eval options...]"
    exit 1
fi
...
OUT_DIR="${LOSS_OUTPUT_DIR:-${SCRIPT_DIR}/loss_output/caption_${CKPT_STEP}_trainlike_${TIMESTAMP}}"
...
cd "${SCRIPT_DIR}"
exec "${OPENPI_PYTHON}" scripts/eval_dataset_loss_trainlike.py \
    "${TRAIN_CONFIG_NAME}" \
    --exp-name "${EXP_NAME}" \
    --step "${CKPT_STEP}" \
