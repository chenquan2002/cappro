...
class PI0:
    def __init__(self, train_config_name, model_name, checkpoint_id, pi0_step, usr_args=None):
        ...
        if self.use_support_context:
            ...
            support_bank_root = usr_args.get("support_bank_root") or os.environ.get("SUPPORT_BANK_ROOT")
            if not support_bank_root:
                robotwin_root = policy_root.parents[1]
                candidates = (
                    robotwin_root / "data" / "support_data" / "support_bank",
                    robotwin_root / "data" / "support_data_full",
                    robotwin_root / "data" / "support_data" / "support_bank_place_fan_clean_ego_v2",
                )
                support_bank_root = next(
                    (
                        path
                        for path in candidates
                        if (path / "human" / self.support_task_name / self.support_task_config).is_dir()
                    ),
                    candidates[0],
                )
            self.support_bank_root = Path(support_bank_root).expanduser().resolve()

            self.support_id = usr_args.get("support_id") or "human_demo_000"
            self.support_view = usr_args.get("support_view") or "ego"
            ...
            if self.mask_support_video:
                ...
                print(
                    f"[Support Eval] found {len(self.support_candidates)} {self.support_view} video candidates "
                    f"under {self.support_bank_root}"
                )
